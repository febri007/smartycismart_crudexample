...
class ApplicationBase extends CI_Controller
{
    ...
    protected function base_load_app()
    {
        ...
        $this->smarty->load_javascript("adminlte31/plugins/bootstrap/js/bootstrap.min.js");
        $this->smarty->load_javascript("adminlte31/js/adminlte.min.js");
        $this->smarty->load_javascript("admin/plugins/fa-iconpicker/dist/js/fontawesome-iconpicker.min.js");
        // // load style
        $this->smarty->load_style("adminlte31/plugins/fontawesome-free/css/all.min.css");
        $this->smarty->load_javascript("admin/plugins/fa-iconpicker/dist/css/fontawesome-iconpicker.min.css");
    ...
    protected function base_view_app()
    {
        ...
        if (!empty($session)) {
            $this->com_user = $this->m_account->get_user_profil([$session['user_id'], 1]);
    ...
    private function _display_sidebar_navigation()
    {
        ...
        $params = [$this->portal_id, 1, $this->parent_selected];
    ...
    private function _get_child_navigation($parent_id)
    {
        ...
        $params = [$this->portal_id, 1, $parent_id];
        $rs_id = $this->m_site->get_navigation_user_by_parent($params);
        if ($rs_id) {
            $html = '<ul>';
            foreach ($rs_id as $rec) {
